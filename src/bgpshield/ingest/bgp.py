...
Path direction: the stored ``as_path`` is **origin first**. See ``bgpshield.paths``.
"""
...
from bgpshield.net import build_session, download
from bgpshield.paths import PathFlag, normalize
...
def fetch_dump(cfg: Config, collector: str, url: str, *, force: bool = False) -> Path:
    ...
    **Why this is not streamed straight into the parser.** Handing a URL to the MRT parser
    ...
    Downloading first removes the ambiguity, because ``bgpshield.net.download`` compares what
