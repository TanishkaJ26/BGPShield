...
reads the Parquet tables written by ``bgpshield ingest-rpki``; nothing downloads.

Three breakdowns, as plan Section 11 asks for: per day, per trust anchor, and per country
of registration. The country caveat from ``bgpshield.ingest.meta`` applies to the third one.
"""
...
def to_json(tables: AdoptionTables, *, top_countries: int = 25) -> dict[str, Any]:
    ...
    return {
        "generated_from": "bgpshield ingest-rpki (RIPE NCC RPKI archive, Routinator JSON)",
        "caveat_country": (
