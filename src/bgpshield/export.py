...
from bgpshield.analysis.longitudinal import build_series
...
def export_networks(cfg: Config, destination: Path) -> Path:
    ...
    meta_path = month_table(cfg, "as_meta", month)
    frame = by_as
    if meta_path.exists():
        frame = frame.join(
...
def export_incidents(cfg: Config, destination: Path) -> Path:
    ...
    Reads what `bgpshield incidents` stored rather than re-fetching any archive window, so the
    ...
    if not stored.exists():
        raise FileNotFoundError("no incident results stored; run 'bgpshield incidents' first")
