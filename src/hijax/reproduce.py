...
run is not an acceptance check. The logic is importable here, `hijax reproduce` runs it with
...
from hijax.tables import read_partition, table_root
...
def run_pipeline(repo: Path, config_path: Path, echo: Any) -> bool:
    ...
    executable = repo / ".venv" / "Scripts" / "hijax.exe"
    command = [str(executable)] if executable.exists() else [sys.executable, "-m", "hijax"]
    iso = str(DAY)
    steps = [
