...
def load_vrp_index(cfg: Config, day: date) -> VrpIndex:
    ...
    if not path.exists():
        raise FileNotFoundError(f"no VRPs ingested for {day}; run 'hijax ingest-rpki' first")
...
def load_aspa_registry(cfg: Config, day: date) -> AspaRegistry:
    ...
    if not path.exists():
        raise FileNotFoundError(f"no ASPAs ingested for {day}; run 'hijax ingest-rpki' first")
