...
def test_a_skipped_export_reports_the_file_it_left_behind(tmp_path: Path) -> None:
    ...
    out = tmp_path / "web"
    out.mkdir()
    (out / "networks.json").write_text('{"snapshot_date": "1999-01-01"}', encoding="utf-8")

    result = build_all(cfg, destination=out)
    assert result.written == []
    assert out / "networks.json" in result.stale
    assert result.total_bytes >= (out / "networks.json").stat().st_size
